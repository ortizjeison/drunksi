# -*- coding: utf-8 -*-
#editable.py
#Editado por Jordi Neil Sánchez
#Iniciado: 15/09/17
#Terminado: 
...
#Importando todas las imágenes (assets) a usar
...
def ubicacion():
    ...
    if(area > 2000000):
        ...
        print "x = ", x
        print "y = ", y
        #cv2.rectangle(imagen, (x, y), (x+50, y+50),(0,0,255), 0,8)
        #cv2.putText(imagen,'Verde',(x-2,y+2), cv2.FONT_HERSHEY_SIMPLEX, 2,(040,166,057),2,5)
        vect1= (2*x,2*y)
    else:
        vect1= pygame.mouse.get_pos()
...
class Fruit:
    ...
    def checkCollide(self):
        if self.fruit_type != "banana":
            #A MODIFICAR
            if collide(self.collide_type, (self.x, self.y), ubicacion(), CollisionRadius[self.fruit_type]) == True:
                return True
            else:
                return False
        else:
            #A MODIFICAR
            if collide(self.collide_type, (self.x, self.y), ubicacion(), self.draw_image.get_width(), self.draw_image.get_height()) == True:
                return True
            else:
...
class Button:
    ...
    def checkHover(self):
        #A MODIFICAR
        if self.rect.collidepoint(ubicacion()):
            return True
        else:
            return False
...
def splashScreen():
    ...
    splashFruits.x = 653 + 225+ 64
    splashFruits.y = 394 + 64
    splashFruits.vx, splashFruits.vy = 0, 0
    # fruit.x = 653 + 225+ 64
    # fruit.y = 394 + 64
    ...
    while running:
        ...
        if splashFruits.checkCollide() == True:
            #fruit.makeSound()
            splashChoice = Bits(splashFruits.fruit_type, splashFruits.x, splashFruits.y, splashFruits.angle, uniform(0.5, 1.5))
        ...
        #A MODIFICAR        
        blade.drawBlade(screen, ubicacion())
...
def main():
    ...
    while running:
        ...
        if counter >= choice ([5, 10, 10, 20, 20, 20, 30, 30, 50, 50, 50, 100, 100, 150]):
        ...
        counter2 += 1
        if counter2 >= choice ([50, 50, 50, 100, 100, 150, 150, 200]):
            for i in range (choice([1, 1, 1, 1, 1, 2, 2, 2, 3, 3, 4, 5])):
                air.append(Fruit(randfruit()))
                counter2 = 0
